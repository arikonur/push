...
$usr = $row['username'];
    ...
    private $appCurlUrl;
    private $appCurlType = "POST";
    private $appContentType = "application/json";
    private $appCurlPostData = [];

    private function setCurlUrl($curlUrl)
    {
        $this->appCurlUrl = $curlUrl;
    }

    private function setCurlType($curlType)
    {
        $this->appCurlType = $curlType;
    }

    private function setContentType($contentType)
    {
        $this->appContentType = $contentType;
    }

    private function setCurlPostData($fields)
    {
        $this->appCurlPostData = $fields;
    }

    private function SendCurl() {

        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $this->appCurlUrl );
        ...
        curl_setopt($ch, CURLOPT_CUSTOMREQUEST, $appCurlType);
        curl_setopt($ch, CURLOPT_POSTFIELDS, $appCurlPostData);
...
if ($_SESSION['username'] == $usr){?>
    ...
    function sendMessage() {
        ...
        while($row = $result->fetch_assoc()) {
            ...
            $curlAddress="https://onesignal.com/api/v1/notifications";
            $appCurlPostData = json_encode($fields);
            $this->setCurlUrl($curlAddress);
            $this->setCurlType('GET');
            $this->setContentType('application/json');
            $this->SetJobRequestData($curlAddress);
            $result=$this->SendCurl();
            $this->SetJobResponseData($result);
            echo $curlAddress;
            echo $result;
